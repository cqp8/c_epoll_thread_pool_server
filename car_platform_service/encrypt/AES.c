 ...
 * 入口函数1:AES_encrypt(in,out,l,w)，加密l字节
 * 入口函数2:AES_decrypt(in,out,l,w)，解密l字节
 * 入口函数3:key_expansion(key,w)，根据key生成w，用于加密解密，
             当key为16字节的时候，w长度为4*11*4
 */
#include <stdio.h>
#include <stdlib.h>
#include <stdint.h>
...
/*
 * Transformation in the Cipher that processes the State using a non颅
...
int AES_decrypt(unsigned char* in, unsigned char* out,int length,unsigned char* w)
{
    int i;
    int block_num;
    block_num = (length-1)/16+1;

    for(i=0;i<block_num;i++)
    {
        inv_cipher(in+i*AES_BLOCK_SIZE,out+i*AES_BLOCK_SIZE,w);
    }
    return AES_OK;
}
...
int AES_encrypt(unsigned char* in, unsigned char* out,int length,unsigned char* w)
{
    ...
    for(i=0;i<block_num;i++)
    {
        cipher(in+i*AES_BLOCK_SIZE,out+i*AES_BLOCK_SIZE,w);
    }
    return block_num*AES_BLOCK_SIZE;
}
